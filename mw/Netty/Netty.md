...
推荐通过 Unpooled 类来创建一个新的 ByteBuf 实例。
...
任何名称以 read 或 skip 开头的操作都会获取或跳过当前 readerIndex 处的数据

**数据写入**

任何名称以 write 开头的操作都会在当前 writerIndex 处写入数据，如果没有足够的可写字节，会引发 IndexOutOfBoundsException。
...
连接到网络套接字或能够执行 I/O 操作（例如读取、写入、连接和绑定）的组件。可以对 Channel 进行以下操作：
