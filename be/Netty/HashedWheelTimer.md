...
- 内部一个线程 TimerThread 循环取“最早到期任务”（小顶堆堆顶）。
- 到期就执行，周期任务执行完再重新计算下次时间放回堆里。

我们先来看下 Timer 是如何使用的，示例代码如下。
...
DelayedQueue 是 JDK 中一种可以延迟获取对象的阻塞队列，其内部是采用优先级队列 PriorityQueue 存储对象。DelayQueue 中的每个对象都必须实现 Delayed 接口，并重写 compareTo 和 getDelay 方法。

- 本质是按到期时间排序的阻塞队列。
- 你 put 任务进去；消费者线程 take 时，未到期就阻塞，到期才拿到。
- 它只管“排队和到期判断”，不管“谁来执行”。
...
为了解决 Timer 的设计缺陷，JDK 提供了功能更加丰富的 ScheduledThreadPoolExecutor。

- 优点：多线程、稳定、功能完整（延迟/周期）、可控性好。
- 缺点：任务量极大时，堆结构插入/删除是 O(log n)，可能成为瓶颈。

ScheduledThreadPoolExecutor 提供了周期执行任务和延迟执行任务的特性，下面通过一个例子先看下 ScheduledThreadPoolExecutor 如何使用。
...
ScheduledThreadPoolExecutor 在 ThreadPoolExecutor 的基础上，重新设计了任务 ScheduledFutureTask 和阻塞队列 DelayedWorkQueue。ScheduledFutureTask 继承于 FutureTask，并重写了 run() 方法，使其具备周期执行任务的能力。DelayedWorkQueue 内部是优先级队列，deadline 最近的任务在队列头部。对于周期执行的任务，在执行完会重新设置时间，并再次放入队列中。ScheduledThreadPoolExecutor 的实现原理可以用下图表示。
...
ScheduledThreadPoolExecutor
        ...
        v
+------------------------+
|   DelayedWorkQueue     |  (优先级队列，最近 deadline 在队头)
+-----------+------------+
            |
            v
     +------+------+
     ...
     +------+------+
            |
            +--> run one-shot task -> 完成即移除
            |
            +--> run periodic task -> 计算下一次 deadline -> 重新入队
```

以上我们简单介绍了 JDK 三种实现定时器的方式。可以说它们的实现思路非常类似，都离不开**任务**、**任务管理**、**任务调度**三个角色。三种定时器新增和取消任务的时间复杂度都是 O(logn)，面对海量任务插入和删除的场景，这三种定时器都会遇到比较严重的性能瓶颈。

因此，对于性能要求较高的场景，我们一般都会采用时间轮算法。那么时间轮又是如何解决海量任务插入和删除的呢？

- 上面三者核心都偏“堆/优先队列”思路，新增/调整大量定时任务时成本更高。
- 时间轮把任务分桶，插入/取消通常可做到接近 O(1)（摊还），更适合海量、精度要求没那么极致的超时场景。
...
任务映射示例：
- delay=3s   -> (2+3)%8  = 5  -> slot[5], round=0
- delay=12s  -> (2+12)%8 = 6  -> slot[6], round=1

slot[6] 链表示意：
head -> taskA(round=0) -> taskB(round=1) -> taskC(round=2) -> null
```
...
时间轮中的删除（取消）原理也比较直接：任务会先被标记为取消状态，然后在后续 tick 推进过程中由工作线程统一清理。这样做可以避免在多线程并发下频繁直接修改 slot 链表，减少锁竞争和复杂同步开销。是否真正可取消取决于任务状态：如果任务还未进入执行阶段，通常可以被成功取消；如果任务已经到期并进入执行流程，则取消会失败（或不生效）。
...
```text
HashedWheelTimer 内部结构（简化）

                +-----------------------------+
                |      HashedWheelTimer       |
                | tickDuration / mask / wheel |
                +--------------+--------------+
                               |
                               v
                       +---------------+
...
bucket -> timeout <-> timeout <-> timeout
           (deadline, remainingRounds, TimerTask)
```
...
1. 精度有限
   - 由 tickDuration 决定，不能保证精确到达时刻执行。
   - 任务通常是“不早于 deadline”，可能晚执行。

2. 单线程执行任务
   - Worker 串行跑任务。
   - 某个任务耗时长，会阻塞后续任务，产生堆积。

3. 空推进开销
   - 即使当前没有到期任务，时间轮也会按 tick 推进。
   - 长时间空转会有额外 CPU 开销。

4. 内存占用偏大
   - 需要维护 wheel/bucket/timeout 链表结构。
   - 任务很多时对象数量大，GC 压力会上升。

5. 不适合“超大跨度 + 高精度”场景
  - Netty 这版是单层时间轮，面对跨度特别大的任务不如层级时间轮（如 Kafka）灵活。

6. 取消不是立即强一致
   - 常见是先标记取消，再由 worker 后续清理。
   - 如果任务已进入执行阶段，取消不会中断正在执行的任务。

结论：它非常适合“海量、近实时、可接受一定延迟”的超时类任务；不适合重 CPU 任务和高精度调度。
...
1. 推进方式
   - Netty HashedWheelTimer：worker 线程按 tickDuration 周期推进。
   - Kafka：用 DelayQueue 管 bucket，到期 bucket 才被取出推进，减少空转。

2. 层级结构
   - Netty：通常单层 wheel。
   - Kafka：多层 wheel（类似秒针/分针/时针），任务过远会放上层，临近再下沉到下层（下层更细粒度）。

3. 时间跨度处理
   - Netty：跨度很大时会经历更多 round，空推进更明显。
   - Kafka：通过分层把“大跨度任务”放高层，避免低层长时间空跑。

4. 任务组织
   - Netty：bucket 内链表 + remainingRounds。
   - Kafka：bucket（TimerTaskList）作为 DelayQueue 元素，按 bucket 到期批量处理。

Kafka 对时间轮的改进点

1. 引入 DelayQueue 作为“推进器”
   - 解决单纯固定 tick 的空推进问题。

2. 引入层级时间轮（Hierarchical Timing Wheel）
   - 更好支持“短延迟 + 超长延迟”混合场景。
   - 下沉/升级机制让时间粒度和跨度兼顾。

3. 更偏向大规模调度场景
   - 在高任务量、跨度大的场景，整体调度效率和资源利用率更稳。
...
Kafka 时间轮 + DelayQueue（简化）
  ...
  +---+---+             +---+---+             +---+---+
      |                     |                     |
      v                     v                     v
TimerTaskList          TimerTaskList         TimerTaskList
  e1 <-> e2              e3 <-> e4             e5 <-> e6
```
...
L1: tick=1ms,   wheelSize=20, interval=20ms
    [0][1][2]...[19]
      ^
    current

L2: tick=20ms,  wheelSize=20, interval=400ms
    [0][1][2]...[19]
      ^

L3: tick=400ms, wheelSize=20, interval=8000ms
    [0][1][2]...[19]
      ^

升级/降级规则：
- deadline 超出当前层 interval -> 放入上层
- 上层 bucket 到期下沉 -> 重新分配到下层
```
