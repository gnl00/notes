...
> 在 JDK 1.7 之前，字符串常量池被放在**方法区**（HotSpot 中通常对应永久代）。永久代空间受限，在大量使用字符串的场景下更容易出现 OOM。
> 
...
新生代分配不了的大对象或到达一定岁数后的对象都会进入老年代。老年代空间压力较大时可能触发 **FGC（Full GC）**。FGC 往往伴随较长停顿，且通常会涉及整个堆（具体行为与收集器实现有关）。FGC 后若空间依然不足会报 OOM：
