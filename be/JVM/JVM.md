...
> **JDK/JRE/JVM**
>
> * JDK（*Java Development Kit*），Java 开发工具。包含编译/调试工具（javac/java/jdb 等）以及运行时组件（JDK 8 常见为 JRE + `rt.jar`；JDK 9+ 使用模块化运行时，如 `java.base`）；
>
...
> **联系**
>
> - JVM 不能单独完成 `.class` 的运行。解析 `.class` 文件时 JVM 还需要依赖标准类库和运行时组件（JDK 8 常见目录含 `jre/lib`）。
>
...
> **区别**
>
> - 在 jdk/bin 目录下有 `javac` 等工具，而运行时环境主要提供 `java` 等命令。`javac` 是将 Java 文件编译成字节码文件的工具，属于开发能力；这也说明 JDK 是开发环境，JRE 是运行时环境（JDK 9+ 不再单独提供独立 JRE 发行包）；
> - 仅 JVM 自身还不能执行 `.class` 文件，还需要标准类库支持（JDK 8 典型是 `rt.jar`，JDK 9+ 为模块化类库）。
...
这也是为什么 `final static` 修饰的变量必须在定义处或静态代码块中初始化，且只能被赋值一次，因为它们在类初始化阶段就已经被确定了其值，而无法在后续步骤中改变。
...
可以看到，除了 main 方法，其中还多了 `<init>()` 和 `<clinit>()` 两个方法，其中：

* 初始化阶段就是执行类初始化方法 `<clinit>()` 的过程。此方法不需要定义，是 `javac` 编译器自动收集类中所有类变量的赋值动作和静态代码块中的语句合并而来。如果类中没有定义静态变量或静态代码块，不会有此方法。
* 和 `<clinit>()` 不一样，`<init>()` 表示实例构造方法，用于实例初始化。

在类加载过程中，如果类具有父类，会先初始化其父类。JVM 保证子类 `<clinit>()` 执行之前，父类的 `<clinit>()` 已经执行完毕。如果该类实现了任何接口，则按顺序初始化这些接口。

此外，为了保证多线程环境下类加载的安全性，JVM 必须保证一个类的 `<clinit>()` 在多线程下被同步执行。
...
*Bootstrap ClassLoader*，由 C/C++ 实现，用来加载 Java 核心类库（JDK 8 常见是 `rt.jar`；JDK 9+ 主要来自模块如 `java.base`），用于加载 JVM 自身需要的类。

启动类加载器<mark>没有父加载器</mark>，是扩展类和系统类加载器的父加载器。出于安全考虑，启动类加载器只加载核心类库（如 java.* 等）。

<br>

##### 扩展类加载器

*Extension ClassLoader*，由 Java 实现。JDK 8 中常见实现类为 `sun.misc.Launcher$ExtClassLoader`，是 `sun.misc.Launcher` 的匿名内部类。继承 `java.lang.ClassLoader`。

负责从 `java.ext.dirs` 所指定目录加载类库（JDK 8 语义）。JDK 9+ 移除了扩展机制，改为 Platform ClassLoader。
...
- 采用双亲委派机制的可以**避免类的重复加载**。比如加载 `java.lang.Object` 时，不管是请求哪个类加载器，都会层层向上委托到启动类加载器进行加载。这样就保证使用不同的类加载器最终得到的都是同一个 `Class` 对应的 `Object` 类型；
...
自定义一个名为 String 类，定义并执行 main 方法，在 JVM 加载自定义的 String 类时会率先使用启动类加载器进行加载，而启动类加载器在加载过程中会先加载 JDK 自带的 `java.lang.String`。
...
如果一个类是由用户自定义类加载器加载，那么 JVM 就会将这个类加载器的一个引用作为类信息的一部分保存到方法区中。当解析一个类到另一个类的引用时，JVM 需要保证这两个类的类加载器约束是可见且一致的。
...
被动使用（通常不初始化）：除了主动使用外的其他 Java 类的使用都可以看作是类的被动使用。

- 发生了“执行该类静态逻辑的必要动作”（new、static读写、static调用、反射主动forName）=> 主动使用
- 只是“拿到类型信息或间接引用” => 多数是被动使用
...
> 方法区是 JVM 规范中的逻辑区域，并非 HotSpot 独有。对于 HotSpot，可将方法区理解为规范层概念，其历史实现经历了永久代到元空间的变化，可以将方法区理解为一个接口，永久带和元空间都是它的实现。
...
> 在 JDK 1.7 之前，字符串常量池被放在**方法区**（可以认为方法区是一个接口/规范，永久代和元空间都是它的实现）。永久代空间受限，在大量使用字符串的场景下更容易出现 OOM。
>
> JDK 1.7 及以后，**字符串常量池移至了堆区**，字符串的创建也在堆区。为了节省开支 `String#intern` 不再是把该字符串直接加入字符串常量池，而是将其地址引用放到字符串常量池。
>
...
- <mark>栈不存在垃圾回收的问题，线程结束即释放内存</mark>；常见报错有 `StackOverflowError/OutOfMemoryError`，前者常见于深层递归导致栈耗尽，后者常见于线程创建过多导致栈内存不足。
- 局部变量（含基本类型与对象引用）保存在栈帧的局部变量表中；对象实例本身通常分配在堆中；
- 每个方法被执行时，都会创建一个<mark>栈帧</mark>，将栈帧压入栈中，当<mark>方法正常返回或是抛出未捕获的异常</mark>时，栈帧随之出栈。

```text
Thread A Java Stack (private)
+--------------------------------------+
| top                                  |
| +----------------------------------+ |
| | Frame: methodC (current)         | |
| | - locals: this, x, y             | |
| | - operand stack: ...             | |
...
用于存放对象实例，绝大多数对象实例都在堆中分配内存。

> 为什么说“绝大多数”而不是“全部”？
>
> 这是因为 JIT 在开启逃逸分析后，可能对部分不会逃逸的方法内对象做标量替换，甚至让对象分配发生在栈上或被优化掉。也就是说，存在少量对象不一定真正落到堆上。

<mark>在 JVM 启动时创建</mark>，一个 JVM 实例只存在一个堆内存（堆内存的大小可调节），属于<mark>**线程共享**的内存区域</mark>。

加载器读取类文件后，会将类元数据放入方法区（HotSpot 8 为永久代/HotSpot 8+ 为元空间），对象实例通常分配在堆中。根据对象生命周期的不同，JVM 会把对象进行分代管理，由垃圾回收器进行分代垃圾回收。

> 堆中没有可用空间时，会抛出 OOM 异常 `Java heap space`。
...
**逻辑上（以 HotSpot 演进视角）** 可理解为：

* 新生代；
* 老年代；
* 方法区实现：Java 7 及以前常见为永久代（*PermGen*），Java 8+ 为元空间（*Metaspace*，使用本地内存）
...
在第一次 GC 开始前，对象只会存在于 Eden 区和幸存者 From 区，幸存者 To 区是空的。当伊甸区空间用完且又需要创建对象时，垃圾回收器就会对伊甸区进行垃圾回收（*Minor GC*）。
**只要产生  GC，伊甸区几乎会被全部清空，剩余的对象转移到幸存者 To 区**。

接着原先的 From 区变成 To 区，而 To 区变成新的 From 区。新对象依旧在伊甸区被创建，如果又发生 GC，幸存对象继续在 From 和 To 中交换，当幸存对象在幸存者区中 From 和 To 来回复制交换次数达到 15 次之后，还存活的对象就存入养老区。次数由 JVM 参数 `MaxTenuringThreshold` 决定，默认为 15。
...
老年代空间压力较大时可能触发 **FGC（Full GC）**。FGC 往往伴随较长停顿（Stop The World），且通常会涉及整个堆（具体行为与收集器实现有关）。
...
* 堆内存设置不够，可通过参数 `-Xms` 和 `-Xmx` 来调整堆内存大小；
...
- `-Xms`，设置初始堆大小，常见默认值约为物理内存的 1/64；
- `-Xmx`，设置最大堆大小，常见默认值约为物理内存的 1/4；
- `-XX:MaxTenuringThreshold`，设置对象在新生代中交换的次数；
- `-XX:PretenureSizeThreshold`：设置大对象直接进入老年代的阈值（避免在年轻代反复复制）
- `-XX:+PrintGCDetails`，输出详细 GC 日志（JDK 8 常用；JDK 9+ 建议使用 `-Xlog:gc*`）。
...
Java 8 中，堆内存结构主要分为新生代和老年代；元空间不在堆中，使用本地内存。
...
> TLAB 只是 Eden 区中的线程本地分配缓冲区，本质仍属于堆内存。对象在 TLAB 分配与在共享 Eden 分配，都是堆内分配。TLAB 解决的是并发分配时的竞争问题，不是“堆外分配”的例外。
...
3、**弱引用**，在 JVM GC 触发时会检查是否存在弱引用对象，弱引用对象只要被**垃圾回收器发现就会回收**；

4、**虚引用**，在 JVM GC 触发时会检查是否存在虚引用对象，虚引用对象只要被**垃圾回收器发现就会回收**。但在被**回收之前**会被放入回收队列 `ReferenceQueue`。其他几种引用是被 JVM 的垃圾回收机制**回收之后**，才放入 ReferenceQueue 中。虚引用大多被用于引用销毁前的处理工作，因此虚引用创建的时候必须带有 ReferenceQueue 参数。

> GC 进行可达性分析（根搜索）过程中，如果发现某个对象仅通过弱引用（WeakReference）或虚引用（PhantomReference）可达（没有强引用或软引用指向它），该对象就会被判定为“不可达”或“可回收”，从而被回收。
...
- 方法区/元空间空间不足。当系统中要加载的类、反射生成的类较多时，可能触发 Full GC 或直接报内存异常（如 `OutOfMemoryError: Metaspace`）。
...
3、调整对象晋升阈值（如 `-XX:MaxTenuringThreshold`）；
...
*GC Roots*（根集合）是可达性分析的起点。垃圾回收过程中，会回收不可由 *GC Roots* 到达的对象。通常有以下对象会被认为是 Root 对象：
...
GC Roots
  ...
  +--> [D] ----> [E]                  (可达，保留)
       ...
       [X] ----> [Y]                  (与 GC Roots 无路径，不可达，可回收)
```

通过一系列 *GC Roots* 对象作为起点进行遍历，如果在 *GC Roots* 和某个对象之间没有可达路径，则表示该对象是不可达的。对象在被真正回收前，可能会经历终结流程（`finalize`）（终结机制在 JDK 9 起被标记为 deprecated）。
...
复制算法需要使用到两块内存区域，每次只用其中一块，当一块用完，就将存活的对象复制到另一块。新生代对象一般存活率较低，所以通常使用两块 10% 的内存作为空闲和活动空间，即 From 和 To 区。另外 80% 的区域则用来给新建对象分配内存，即 Eden 区。
...
- JVM 在回收前会判断对象是否需要执行终结逻辑；
- 若需要且此前未执行过，会将其放入终结队列，由 Finalizer 线程异步执行一次
- 若对象在终结阶段未“复活”，后续 GC 才会真正回收该对象。

> 即使执行了 finalize，也不保证对象一定被复活/保留。finalize() 只是“给对象一次自救机会”，不是“保活承诺”。
>
> 1. 要复活，必须在 finalize() 里把 this 重新赋给某个 GC Roots 可达的位置（如静态变量）。
> 2. finalize() 最多只会被 JVM 自动调用一次。即使这次复活了，下次再变不可达时通常就直接回收，不会再给第二次机会。
...
> 1. Serial；
> 2. Parallel；
> 3. CMS（已在 JDK 9 标记废弃，JDK 14 移除）；
> 4. [G1](https://docs.oracle.com/en/java/javase/17/gctuning/garbage-first-g1-garbage-collector1.html)；
> 5. [ZGC](https://docs.oracle.com/en/java/javase/17/gctuning/z-garbage-collector.html)：JDK 11 引入的低延迟垃圾收集器，目标是把停顿控制在毫秒级（不承诺固定绝对上限），ZGC 收集器可以对整个堆内存进行垃圾收集，并支持大堆场景。
>
...
* ParNew 作用于新生代，是 Serial 收集器的多线程版本（历史上常与 CMS 配合，现代版本已较少使用）。
...
| `-XX:CMSFullGCsBeforeCompaction`     | 在执行多少次不压缩的 FGC 后，进行一次压缩的 FGC。                                                                                                                                                                                                                                                         |
...
G1 是面向服务端的低停顿收集器，在 JDK 9 起成为默认收集器（替代 CMS 的主流选择）。它将堆内存划分为多个独立 Region，并且每个区域的大小相同，可根据垃圾分布优先回收收益更高的区域。
...
`-XX:+PrintGCDetails` 打印 GC 细节（JDK 8 常用；JDK 9+ 建议使用 `-Xlog:gc*`）
