...
> 为什么是几乎所有？
>
> 一个例子：JDK 7 之前字符串常量池在方法区（HotSpot 多对应永久代）；JDK 7+ 字符串常量池移到堆中。
...
通过一系列 *GC Roots* 对象作为起点进行遍历，如果在 *GC Roots* 和某个对象之间没有可达路径，则表示该对象是不可达的。对象在被真正回收前，仍可能经历一次终结流程（如 `finalize`，该机制已废弃且不推荐依赖）。
...
2、对象可能会经历终结流程（`finalize`，已废弃且不推荐依赖）。简化流程如下：
