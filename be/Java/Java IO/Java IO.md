...
> **IO 多路复用技术**
>
> 常见的 IO 模型包括：`阻塞 IO`、`非阻塞 IO`、`IO 多路复用`和`异步 IO`。
>
> * 在阻塞 IO 模型中，线程会一直等待 IO 操作完成
> * 非阻塞 IO 模型中，线程不会等待 IO 操作完成，而是通过**轮询**的方式不断查询 IO 操作状态
...
> <br>
>
> 先介绍 fd_set：是一种数据结构，是一个集合，用于存储被监控的文件描述符（*file descriptor*）。常被用于 IO 操作，比如 IO 多路复用技术中 select/poll/epoll 都使用到了它。
> 
> **IO 多路复用技术使用到的*系统调用方法***
>
> * `select()`
>
>   select 函数是最早的多路复用函数之一，通过传入的 `fd_set` 指定要关注的一组文件描述符，并阻塞等待，看它们是否在“可读/可写/异常”这三类条件上就绪，来实现 IO 多路复用。
>   fd_set（BitMap 结构，默认长度为 FD_SETSIZE=1024）监视的文件描述符数量有限；监视的实现方式是遍历整个 fd_set，时间复杂度为 O(N)。
>   select 每次调用都会把 fd_set 集合从用户态拷贝到内核态，由内核来遍历 fd_set，所以性能较低。此外，select 函数的实现方式要求用户传递 fd_set 集合，所以它存在一定的安全漏洞。
>
> * `poll()`
>
>   poll() 使用 `struct pollfd fds[]` 数组来描述要监视的 fd 集合，没有 `FD_SETSIZE` 的固定上限；但它仍然需要每次调用把数组在用户态/内核态之间拷贝，并在内核中线性扫描这些 fd，时间复杂度为 O(N)。
>
> * epoll
>
>   epoll 函数是 Linux 中的一种 IO 多路复用机制，可监视的文件描述符数量不受限制（不受 FD_SETSIZE 这种结构性限制，但受系统资源限制）。
> * epoll 避免了每次系统调用都拷贝整份监听集合，只拷贝就绪事件：它对就绪 IO 进行了标记，只需要拷贝就绪事件列表到用户态，不再需要对整个 fd_set 进行轮询操作，效率比 select 和 poll 高。
> * epoll 内部使用红黑树来管理 fd_set，减少遍历 fd_set 的时间复杂度，时间复杂度为 O(logN) 。
>
> <br>
...
BIO 是传统的 IO 操作，它是一种同步阻塞的 IO 模式。在执行输入/输出操作时，线程会被阻塞，直到操作完成。这意味着当一个线程在执行 IO 操作时，其他线程将被阻塞，直到该 IO 操作完成。

在 Java 中，BIO 是通过 InputStream 和 OutputStream 来实现的：
...
在上面的代码中，InputStream#read 方法会被阻塞，直到它从文件中读取到数据。由于该操作是同步的，因此该线程将一直处于阻塞状态，直到该操作完成。如果有多个线程执行类似的操作，则这些线程将相互阻塞。
...
> NIO 是可多路复用的非阻塞式 IO。在执行输入/输出操作时，线程不会被阻塞，可以继续执行其他操作。这使得一个线程可以同时处理多个输入/输出操作。

NIO 中的核心组件有三个：Channel（通道）、Buffer（缓冲区）和 Selector（选择器）。Channel 负责传输数据，Buffer 负责存储数据，Selector 负责监听通道上的事件，并将事件分发给对应的线程进行处理。

NIO 使用一个线程处理多个连接，在一个线程中可以监听多个 Channel，这些 Channel 都绑定在同一个 Selector 上，当其中的一个 Channel 有事件时（如可读、可写等），Selector 就会通知对应的线程进行处理。

在 Java 中，NIO 是通过 Channel、Buffer、Selector 来实现的
...
在上面的代码中，FileChannel#read 方法不会阻塞线程。
...
> 定义于 `java.nio`，一个用于存放特定原始类型数据的容器。存储数据容量，数据地址，数据大小限制等。使用 Buffer 类可以缓存数据，在处理数据时更加高效。
>
> **只读 Buffer**
>
> 每一个Buffer 都是可读的，但不是每个 Buffer 都是可写的。可以将 Buffer 定义为只读，对只读缓冲区进行写入操作会报错 ReadOnlyBufferException。
>
> **线程安全**
>
> Buffer 对于多个并发线程的使用是不安全的。如果一个 Buffer 要被一个以上的线程使用，要使用适当的同步方式来控制对 Buffer 的访问。
...
> NIO 数据传输的纽带（通道），定义于 `java.nio.channels`。
...
> 一个 SelectableChannel 对象的多路复用器。阻塞并轮询所有 Channel， 当有感兴趣的事件时通知对应的 Channel。
...
> Selector#open 方法会使用系统默认的 SelectorProvider 创建一个 Selector；也可以使用自定义的 SelectorProvider 来创建 Selector。Selector 被打开后将一直是 open 状态，直到它被关闭。

> 将 SelectableChannel 注册到 Selector 上并通过 SelectionKey 来表示。Selector 维护 3 个 SelectionKey 集合。
>
...
> **Selection 操作**
>
> Selection 操作是用于轮询检查注册在 Selector 上的 Channel 是否有就绪事件（例如读就绪、写就绪等），并将就绪 Channel 的 SelectionKey 添加到已选择键集合中。
>
...
在 BIO 中，IO 读写需要经过一段时间的阻塞，等待获取到数据才返回；与 BIO 的关心返回数据不同，NIO 更关心的是是否有自己感兴趣的事件（读/写事件），有则进行事件操作，没有则返回 0，不会阻塞线程。
...
> AIO（*Asynchronous IO*，异步 IO）是 Java NIO 的扩展，也称为 NIO.2，它是异步非阻塞的 IO 操作。 AIO 相对于传统的 BIO（Blocking IO）和 NIO（Non-blocking IO）都具有更高的效率和可靠性。
...
> …
>
> [AIO 扩展阅读](https://mp.weixin.qq.com/s/zAh1yD5IfwuoYdrZ1tGf5Q)。
>
> …
>
> 在 Java 中，AIO 的实现主要是基于 Java NIO 中
>
> * AsynchronousServerSocketChannel
> * 和 AsynchronousSocketChannel
>
> 通过注册 IO 操作和实现 CompletionHandler 接口来实现异步 IO。
